  ...
  return (
    <div className="h-full min-h-0 max-w-full max-h-full overflow-hidden bg-[var(--color-bg)] text-slate-200 flex flex-col">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 md:hidden" aria-hidden>
          ...
          <nav className="absolute top-14 left-0 right-0 mx-4 bg-surface border border-slate-700 rounded-lg shadow-xl py-3 safe-area-inset-top">
            {navLinks.map((l) => (
              <Link key={l.label} to={l.to} className="block px-4 py-3 text-slate-200 hover:text-accent hover:bg-slate-800/50 transition-colors" onClick={() => setMenuOpen(false)}>
                {l.label}
              </Link>
            ))}
      ...
      <main id="main-scroll" className="flex-1 min-h-0 pt-14 sm:pt-16 min-w-0 overflow-auto">
        <Outlet />
        <footer className="px-4 sm:px-6 py-4 sm:py-6 border-t border-slate-800/50 flex flex-row flex-wrap items-center justify-between gap-3 sm:gap-6 text-sm text-slate-500 safe-area-inset-bottom">
